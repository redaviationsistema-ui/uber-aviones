...
class ClienteControlador extends ControladorBase
{
    ...
    public function storeFlightRequest(Request $request)
    {
        ...
        return $this->ok([
            'flight_request' => $this->visibilidadServicio->solicitudParaCliente($solicitud->fresh(['matches.aircraft'])),
            'chat_id' => $chat->id,
        ], 201);
    }
    ...
    {
        $solicitudes = SolicitudVuelo::with(['matches.aircraft'])
    ...
    public function showFlightRequest(Request $request, SolicitudVuelo $flightRequest)
    {
        ...
        return $this->ok([
            'flight_request' => $this->visibilidadServicio->solicitudParaCliente($flightRequest->load(['matches.aircraft'])),
        ]);
    }
